...
download('https://github.com/mbamobi/mba-componentes/archive/master.zip', '.tmp', { extract: true }).then(() => {
  ...
  console.log(path.join('.tmp/mba-componentes-master', args[2], 'src'));
  shelljs.cp('-R', path.join('.tmp/mba-componentes-master', args[2], 'src') + '/*', 'src');
  try {
    console.log('Instalando dependências...');
    var packageJSON = require(path.join('../../.tmp/mba-componentes-master', args[2]) + '/package.json');
    ...
    if (devDependencies) {
      for (var idx in devDependencies) {
        shelljs.exec('npm install --save ' + idx + '@' + devDependencies[idx]);
      }
    }
  } catch(e) {
